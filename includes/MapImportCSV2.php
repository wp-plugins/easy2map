...
if (is_uploaded_file($_FILES["csvfile"]['tmp_name'])) {
    ...
    if (!empty($handle)) {
        ...
        while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
            ...
            if ($num == 4) {
                $address = urlencode(str_replace(';', ',', trim($data[0])));
                $title = trim(urldecode($data[1]));
                $pinIcon = trim($data[2]);
                $pinHTML = trim(urldecode($data[3]));

                $loc = Easy2Map::getLocation($address);
                
                if (is_array($loc) && count($loc) == 2 && array_key_exists("lat", $loc) && array_key_exists("lng", $loc)) {
                    ...
                    PinImageURL,
                    DetailsHTML)
                    VALUES (%s, '%s', '%s', '%s', '%s', '%s');", $mapID, $current_user->ID, "(" . $loc["lat"] . "," . $loc["lng"] . ")", $title, $pinImage, $pinHTML);
...
<div class="wrap" id="bodyTag" style='width:100%;text-align:center'>
    ...
    <form name="formImport4" enctype="multipart/form-data" id="formImport4"
        ...
        <table style="background-color:#EBEBEB;width:60%;margin-left:auto;margin-right:auto;margin-top:10px;" cellspacing="3" cellpadding="3" class="table table-bordered">
            <tr>
                <td class="instructions"><h5>Import Markers via .CSV (Using Address)</h5>
                </td>
            </tr>
        ...
        <table style="font-size:11px;background-color:#FFFFFF;width:60%;margin-left:auto;margin-right:auto;margin-top:30px;" cellspacing="3" cellpadding="3" class="table table-bordered">
            ...
            <tr><td>[marker address],[marker 1 title],[marker 1 icon],[marker 1 description]<br>
            [marker address],[marker 2 title],[marker 2 icon],[marker 2 description]</td></tr>
            <tr><th>

            Example:
            <p style="font-style:italic">Trafalgar Square; London; WC2N; UK, Trafalgar Square, 222.png, At the heart of London<br>
            5 Avenue Anatole France; 75007; Paris; France, Tour Eiffel, 222.png, At the heart of Paris
            </p></th></tr>
            <tr>
                ...
                <td style="margin-top:20px;">
                    ...
                    <ul>
                        <li>Please separate address sections with <b>semi-colons</b> (;) and NOT commas.</li>
